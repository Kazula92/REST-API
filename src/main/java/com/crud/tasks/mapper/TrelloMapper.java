...
public class TrelloMapper {
    ...
    public List<TrelloList> mapToList(final List<TrelloListDo> trelloListDto) {
        return trelloListDto.stream()
                .map(trelloList -> new TrelloList(trelloList.getId(), trelloList.getName(), trelloList.isClosed()))
                .collect(toList());
    }

    public List<TrelloListDo> mapToListDto(final List<TrelloList> trelloLists) {
        return trelloLists.stream()
                .map(trelloList -> new TrelloListDo(trelloList.getId(), trelloList.getName(), trelloList.isClosed()))
                .collect(toList());
    }
